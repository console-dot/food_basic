...
export const ViewPurchase = () => {
  ...
  const getData = async () => {
    try {
      const res = await fetch(
        "https://api-food-basic.vercel.app/api/v1/purchase/daily-all-purchase"
      );
      if (!res.ok) throw new Error(`HTTP error! Status: ${res.status}`);
      const data = await res.json();
      setPurchaseItems(data?.data?.dailySales || []);
      setFilteredItems(data?.data?.dailySales || []);
  ...
  const handelView = async (dateObj) => {
    try {
      const res = await fetch(
        `https://api-food-basic.vercel.app/api/v1/purchase/daily-purchase/${dateObj}`
      );
      if (!res.ok) throw new Error(`HTTP error! Status: ${res.status}`);
      const data = await res.json();
      setView(true);
  ...
  return (
       <div className="min-h-screen bg-[#E8E8E8] p-4 relative">
            ...
            {/* Date Picker */}
            <div className="flex md:flex-row flex-col justify-center items-center my-8">
